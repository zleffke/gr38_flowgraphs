...
class jove_dual_record_sigmf(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, ant_pol_x='N-S', ant_pol_y='E-W', ant_type='KJ4QLP Active HF Dipole Balun', db_type='LFRX', decim=5, geo_alt=655.0, geo_lat=37.206756, geo_lon=-80.418876, interp=1, path="/captures/wwv", samp_rate=12.5e6, signal_type='JOVE', usrp_addr="addr=10.41.1.11", usrp_clock_source="Internal", usrp_subdev_spec='A:A A:B', usrp_sync="Unknown PPS", usrp_time_source="O/B GPSDO", usrp_type='N210'):
        ...
        self.settings = Qt.QSettings("GNU Radio", "jove_dual_record_sigmf")
        ...
        self.top_grid_layout.addWidget(self._qtgui_freq_sink_x_0_win, 5, 0, 4, 4)
        for r in range(5, 9):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
...
def argument_parser():
    ...
    parser.add_argument(
        "--usrp-clock-source", dest="usrp_clock_source", type=str, default="Internal",
    ...
    parser.add_argument(
        "--usrp-time-source", dest="usrp_time_source", type=str, default="O/B GPSDO",
...
def main(top_block_cls=jove_dual_record_sigmf, options=None):
    if options is None:
